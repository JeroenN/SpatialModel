    ...
    m_seagrass_widget{new ribi::mb::QtMutualismBreakdownerSpatialWidget(10,10)}
{
    ...
    QObject::connect(ui->spinBox_traits_mean, SIGNAL(valueChanged(double)), this, SLOT(CreateGrid()));
    QObject::connect(ui->spinBox_traits_dev, SIGNAL(valueChanged(double)), this, SLOT(CreateGrid()));
    QObject::connect(ui->box_fit_fac_opt, SIGNAL(valueChanged(double)), this, SLOT(CreateGrid()));
    QObject::connect(ui->box_fit_fac_sd, SIGNAL(valueChanged(double)), this, SLOT(CreateGrid()));
    QObject::connect(ui->box_fit_unfac_opt, SIGNAL(valueChanged(double)), this, SLOT(CreateGrid()));
    QObject::connect(ui->box_fit_unfac_sd, SIGNAL(valueChanged(double)), this, SLOT(CreateGrid()));
    QObject::connect(ui->spinBox_generation, SIGNAL(valueChanged(int)), this, SLOT(CreateGrid()));

    CreateGrid();

    QObject::connect(ui->spinBox_generation, SIGNAL(valueChanged(int)), this, SLOT(showGraphs()));
     QObject::connect(ui->spinBox_init_n_seeds, SIGNAL(valueChanged(int)), this, SLOT(showGraphs()));
    QObject::connect(ui->spinBox_traits_dev, SIGNAL(valueChanged(double)), this, SLOT(ShowGraphs()));
    QObject::connect(ui->spinBox_traits_mean, SIGNAL(valueChanged(double)), this, SLOT(ShowGraphs()));
...
/*void MainWindow::create_new_facilitated_plants(plant_coordinats &facilitated_plant_coordinates, yx_grid& g)
{
        ...
        for(int j=0; j<nFacilitated; ++j)
        {
            ...
            int x = (rand() % 7*mGeneration) - 3*mGeneration;
            int y = (rand() % 7*mGeneration) - 3*mGeneration;
            new_position_x +=x;
            new_position_y +=y;
            g[new_position_y][new_position_x]=white;
